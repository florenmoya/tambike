...
import { canFulfillGiveawayClaim } from "../../src/features/giveaways/giveaway-claim-scanner-panel";
...
describe("giveaway delivery claim operator UI", () => {
  test("gates only delivery fulfilment on a details view for the same active award", () => {
    const deliveryClaim = {
      awardId: "award-1",
      status: "verified",
    ...
    expect(canFulfillGiveawayClaim(deliveryClaim, null)).toBe(false);
    expect(canFulfillGiveawayClaim(deliveryClaim, { awardId: "award-2" })).toBe(false);
    expect(canFulfillGiveawayClaim(deliveryClaim, { awardId: "award-1" })).toBe(true);
    expect(canFulfillGiveawayClaim({ awardId: "award-3", status: "verified", fulfilmentMode: "onsite" }, null)).toBe(true);
  });
