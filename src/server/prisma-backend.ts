...
import { BackendError, type AuditAction, type RegistrationInput } from "./backend";
...
export class PrismaTambikeBackend {
  ...
  async getSnapshot(sessionToken?: string) {
    ...
    const [users, events, currentPasses] = await Promise.all([
  ...
  async scanPass(
    ...
    await this.audit("CHECK_IN_CREATED", scanner.id, "CheckIn", updated.id);
  ...
  async exportAttendeesCsv(sessionToken: string, eventId: string) {
    ...
    for (const rsvp of rsvps) {
      const checkIn = rsvp.pass
        ? await this.prisma.checkIn.findFirst({ where: { passId: rsvp.pass.id } })
        : null;
      rows.push(
        [
          event.id,
  ...
  async listEvents(query?: EventQueryInput) {
    ...
    return filterEventsByQuery(
      events.map((event) => this.toEvent(event)),
      query,
    );
  }
